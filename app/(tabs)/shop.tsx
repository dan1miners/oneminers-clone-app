...
const BannerCarousel = () => {
  ...
  const renderBannerItem = useCallback(
    ({ item, index }: { item: BannerItem; index: number }) => (
      <View
        className="rounded-2xl overflow-hidden mr-0"
        style={{
      ...
      >
        <View className="flex-1 flex-row p-5">
          <View className="flex-1 justify-center">
            ...
            <TouchableOpacity className="bg-white dark:bg-slate-900 rounded-[20px] px-4 py-2 self-start">
              <Text className="text-sm font-semibold text-black dark:text-slate-100">
                Shop Now
              </Text>
            </TouchableOpacity>
          </View>
          <View className="w-[120px] h-[120px] bg-white dark:bg-slate-900/20 rounded-xl justify-center items-center self-center">
  ...
  const renderDot = useCallback(
    (_: BannerItem, index: number) => {
      ...
      return (
        <Animated.View
          key={index}
          className="h-2 rounded bg-om-accent mx-1"
          style={{
            width: dotWidth,
            opacity: opacity,
          }}
...
const ProductCard = React.memo<{ item: ProductItem }>(({ item }) => (
  <Link href={`/(shop)/product/${item.id}`} asChild>
    <TouchableOpacity className="w-[48%] rounded-xl p-3 mb-3">
      <View className="w-full h-[120px] bg-gray-100 dark:bg-slate-800 rounded-lg justify-center items-center mb-2">
        <Text className="text-4xl">{item.image}</Text>
      </View>
      <View className="flex-1">
        <Text
          className="text-sm font-semibold text-gray-900 dark:text-slate-100 mb-1.5"
          numberOfLines={1}
        >
          {item.name}
        </Text>
        ...
        <View className="flex-row justify-between items-center mb-1">
          ...
          <Text className="text-xs text-om-success font-semibold">
            {item.profit}
          </Text>
        </View>
        ...
        <View className="flex-row justify-between items-center">
          ...
          <Text
            className="text-[11px] text-gray-400 dark:text-slate-500 flex-1 text-right ml-1"
            numberOfLines={1}
          >
            {item.hashrate}
          </Text>
...
const CategoryItem = React.memo<{
  ...
  <TouchableOpacity
    ...
    activeOpacity={0.7}
  >
    <View className="w-[60px] h-[60px] rounded-full bg-gray-100 dark:bg-slate-800 items-center justify-center mb-2">
      <Text className="text-[28px]">{item.icon}</Text>
    </View>
    <Text className="text-xs text-gray-900 dark:text-slate-100 font-medium text-center">
      {item.name}
    </Text>
  </TouchableOpacity>
));
...
export default function ShopScreen() {
  ...
  const handleSearchSubmit = useCallback(() => {
    if (searchQuery.trim()) {
      ...
      router.push(
        `/(essentials)/search/${encodeURIComponent(searchQuery.trim())}`,
      );
      ...
      <View className="px-4 py-3 border-b border-gray-200 dark:border-slate-700">
        <View className="flex-row items-center justify-between">
            ...
            {showSearchBar && (
              <View className="flex-1">
                <View className="flex-row items-center bg-gray-100 dark:bg-slate-800 rounded-xl px-3 h-10">
                  ...
                  />
                  <TextInput
                    className="flex-1 text-base ml-2 mr-2 text-black dark:text-slate-100 py-1"
          ...
          <View className="flex-row items-center">
            <TouchableOpacity
              className="w-9 h-9 rounded-xl bg-gray-100 dark:bg-slate-800 items-center justify-center ml-2"
              onPress={handleSearchPress}
            >
              <Ionicons
                name={
                  showSearchBar && searchQuery ? "search" : "search-outline"
                }
                size={20}
                color={
                  showSearchBar && searchQuery ? APP_COLORS.info : colors.text
                }
              />
            </TouchableOpacity>
            <Link href="/cart" asChild>
              <TouchableOpacity className="w-9 h-9 rounded-xl bg-gray-100 dark:bg-slate-800 items-center justify-center ml-2">
