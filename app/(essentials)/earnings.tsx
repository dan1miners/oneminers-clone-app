...
const MOCK: Record<Period, Row[]> = {
  Hourly: [
    ...
    { id: 'h2', label: '01:00', amountBtc: 0.000010, amountUsd: 0.66 },
    { id: 'h3', label: '02:00', amountBtc: 0.000013, amountUsd: 0.84 },
    { id: 'h4', label: '03:00', amountBtc: 0.000011, amountUsd: 0.72 },
    { id: 'h5', label: '04:00', amountBtc: 0.000014, amountUsd: 0.90 },
  ],
  Daily: [
    { id: 'd1', label: '2025-12-31', amountBtc: 0.002154, amountUsd: 45.30 },
    { id: 'd2', label: '2025-12-30', amountBtc: 0.002041, amountUsd: 43.10 },
    { id: 'd3', label: '2025-12-29', amountBtc: 0.001980, amountUsd: 41.80 },
    { id: 'd4', label: '2025-12-28', amountBtc: 0.002220, amountUsd: 46.90 },
    { id: 'd5', label: '2025-12-27', amountBtc: 0.002080, amountUsd: 44.00 },
  ],
  Monthly: [
    ...
    <SafeAreaView className="flex-1 bg-gray-50" edges={['top']}>
      ...
      <View className="flex-row items-center px-4 py-3 bg-white border-b border-gray-200">
        ...
        <View className="flex-1">
          ...
          <Text className="text-xs text-gray-400 mt-0.5">Earnings</Text>
        </View>
      </View>
      ...
      <View className="px-4 pt-4">
              ...
              return (
                <TouchableOpacity
                  ...
                  className={`mr-2 px-4 py-2 rounded-full border ${
                    active ? 'bg-[#FFC000] border-[#FFC000]' : 'bg-white border-gray-200'
                  }`}
                >
                  <Text className={`text-sm font-semibold ${active ? 'text-black' : 'text-gray-600'}`}>
                    {t}
                  </Text>
                </TouchableOpacity>
              );
        ...
        <View className="flex-row items-center justify-between mt-3 mb-3">
          <View className="flex-row items-center">
            ...
            <TouchableOpacity
              ...
              className={`px-3 py-2 rounded-full border mr-2 ${
                sortKey === 'date' ? 'bg-white border-gray-300' : 'bg-gray-50 border-gray-200'
              }`}
            >
              <Text className="text-xs font-semibold text-black">Date</Text>
            </TouchableOpacity>

            <TouchableOpacity
              onPress={() => setSortKey('amount')}
              className={`px-3 py-2 rounded-full border ${
                sortKey === 'amount' ? 'bg-white border-gray-300' : 'bg-gray-50 border-gray-200'
              }`}
            >
              <Text className="text-xs font-semibold text-black">Amount</Text>
            </TouchableOpacity>
          </View>
          ...
          >
            <Ionicons name={asc ? 'arrow-up' : 'arrow-down'} size={14} color="#111827" />
            <Text className="text-xs font-semibold text-black ml-2">
        ...
        <View className="bg-white rounded-2xl p-4 border border-gray-100 mb-3">
          ...
          <Text className="text-xl font-extrabold text-black mt-1">
            ${totalUsd.toFixed(2)}
          </Text>
      ...
      <View className="flex-1 px-4">
        ...
        <View className="flex-row items-center bg-gray-100 rounded-xl px-3 py-2">
          <Text className="flex-1 text-[11px] font-semibold text-gray-600">
            {period === 'Hourly' ? 'Time' : 'Period'}
          </Text>
          <Text className="w-24 text-right text-[11px] font-semibold text-gray-600">
            BTC
          </Text>
          <Text className="w-24 text-right text-[11px] font-semibold text-gray-600">
            USD
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View className="flex-row items-center bg-white rounded-xl px-3 py-3 mb-2 border border-gray-100">
              <Text className="flex-1 text-sm font-semibold text-black">
                {item.label}
              </Text>

              <Text className="w-24 text-right text-xs text-gray-600">
                {item.amountBtc.toFixed(6)}
              </Text>

              <Text className="w-24 text-right text-xs font-semibold text-black">
