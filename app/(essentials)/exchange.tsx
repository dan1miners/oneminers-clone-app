...
export default function ExchangePage() {
  ...
  const [showAssetPicker, setShowAssetPicker] = useState<null | 'from' | 'to'>(null);
  ...
  const currentExchangeRate = useMemo(() => {
    return exchangeRates.find((r) => r.from === fromAsset && r.to === toAsset) || null;
  }, [fromAsset, toAsset]);
  ...
  const handleExchange = () => {
    ...
    if (amt < currentExchangeRate.minAmount) {
      showError(`Minimum amount is ${currentExchangeRate.minAmount} ${fromAssetData?.symbol}.`);
      return;
    }
  ...
  const renderAssetRow = ({ item }: { item: CryptoAsset }) => {
    ...
    const disabled = isSelectingFrom ? item.id === toAsset : item.id === fromAsset;
  ...
  const renderExchangeItem = ({ item }: { item: ExchangeTransaction }) => {
    const badge =
      item.status === 'Completed'
        ...
        ? { bg: 'bg-[#FFC000]/15', text: 'text-black' }
        : item.status === 'Pending'
        ? { bg: 'bg-yellow-100', text: 'text-orange-600' }
        : { bg: 'bg-red-100', text: 'text-red-700' };

    return (
      <View className="bg-white p-4 rounded-2xl mb-2 flex-row items-start border border-gray-100">
        <View className="flex-1 pr-3">
          <Text className="text-sm font-semibold text-black" numberOfLines={1}>
            - {item.fromAmount} {item.fromAsset.toUpperCase()}
          </Text>
          <Text className="text-sm mt-0.5" style={{ color: COLORS.success }} numberOfLines={1}>
        ...
        <View className={`px-3 py-1.5 rounded-full self-start min-w-[96px] items-center ${badge.bg}`}>
          <Text className={`text-xs font-semibold ${badge.text}`}>{item.status}</Text>
        </View>
      </View>
    );
  };
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-50" edges={['top', 'bottom', 'left', 'right']}>
      ...
      <View className="py-3 px-5 border-b border-gray-200 flex-row items-center h-[60px]">
        ...
        <Text className="text-lg font-bold text-black">Exchange</Text>
      </View>

      <ScrollView className="flex-1 p-4" showsVerticalScrollIndicator={false}>
        {/* Exchange Card */}
        <View className="bg-white rounded-2xl p-5 mb-4 border border-gray-100">
          <Text className="text-lg font-bold text-black mb-4">Quick Exchange</Text>

          {/* From selector */}
          <Text className="text-xs font-semibold text-gray-500 mb-2">From</Text>
          <TouchableOpacity
            activeOpacity={0.85}
          ...
          >
            <View className="flex-row items-center flex-1">
              <View className="w-10 h-10 rounded-full bg-white items-center justify-center mr-3 border border-gray-100">
          ...
          {/* Amount input */}
          <View className="flex-row items-center bg-white border border-gray-100 rounded-2xl mt-3">
            <TextInput
              className="flex-1 px-4 py-4 text-lg font-bold text-black"
              placeholder="0.00"
              value={fromAmount}
          ...
          <View className="items-center my-3">
            <TouchableOpacity
              onPress={swapAssets}
              activeOpacity={0.85}
              className="w-11 h-11 rounded-full bg-[#FFC000]/20 items-center justify-center border border-[#FFC000]/30"
            >
          ...
          {/* To selector */}
          <Text className="text-xs font-semibold text-gray-500 mb-2">To</Text>
          <TouchableOpacity
            activeOpacity={0.85}
          ...
          <View className="flex-row items-center bg-white border border-gray-100 rounded-2xl mt-3">
            <TextInput
              ...
              placeholderTextColor={COLORS.muted}
            />
            <Text className="px-4 text-sm font-bold text-gray-500">
              {toAssetData?.symbol}
            </Text>
          </View>

          {/* Rate info */}
          <View className="mt-4 bg-gray-50 border border-gray-100 rounded-2xl p-4">
            {currentExchangeRate ? (
              <>
                ...
                <View className="flex-row justify-between">
                  <Text className="text-xs text-gray-500">Minimum</Text>
                  <Text className="text-xs font-semibold text-black">
          ...
          <TouchableOpacity
            ...
            className={[
              ...
              !fromAmount || !toAmount || isProcessing ? 'bg-[#E9ECEF]' : 'bg-[#FFC000]',
            ].join(' ')}
          >
        ...
        {/* Recent */}
        <Text className="text-base font-bold text-black mb-3">Recent Exchanges</Text>
        <View className="mb-6">
          <FlatList
            data={mockExchanges}
            ...
            scrollEnabled={false}
            ListEmptyComponent={
              <Text className="text-center text-gray-400 mt-6">No exchange history.</Text>
            }
          />
        </View>
      ...
      <Modal visible={!!showAssetPicker} transparent animationType="fade">
        <View className="flex-1 bg-black/40 justify-center px-5">
          <View className="bg-white rounded-2xl max-h-[80%] border border-gray-100">
        ...
        <View className="flex-1 bg-black/40 items-center justify-center px-5">
          <View className="bg-white rounded-2xl p-5 w-full border border-gray-100">
            <View className="flex-row items-center justify-between mb-2">
              <Text className="text-lg font-bold text-black">Confirm Exchange</Text>
              <TouchableOpacity onPress={() => setShowConfirmModal(false)}>
                <Ionicons name="close" size={20} color="#111827" />
              </TouchableOpacity>
            </View>

            <View className="mt-2 bg-gray-50 rounded-2xl p-4 border border-gray-100">
              <View className="flex-row justify-between mb-2">
                <Text className="text-xs text-gray-500">You send</Text>
                <Text className="text-xs font-semibold text-black">
                  {fromAmount} {fromAssetData?.symbol}
                </Text>
              </View>
              <View className="flex-row justify-between mb-2">
                <Text className="text-xs text-gray-500">You receive</Text>
                <Text className="text-xs font-semibold text-black">
            ...
            <View className="flex-row gap-3 mt-4">
              <TouchableOpacity
                className="flex-1 bg-[#E9ECEF] rounded-2xl py-4 items-center"
              ...
              <TouchableOpacity
                className="flex-1 bg-[#FFC000] rounded-2xl py-4 items-center"
      ...
      <Modal transparent visible={showSuccessModal} animationType="fade">
        <View className="flex-1 bg-black/40 items-center justify-center px-5">
          <View className="bg-white rounded-2xl p-5 w-full border border-gray-100">
            ...
            <TouchableOpacity
              activeOpacity={0.9}
              className="mt-5 bg-[#FFC000] rounded-2xl py-4 items-center"
        ...
        <View className="flex-1 justify-end">
          <View className="px-5 pb-8">
            <View className="bg-white rounded-2xl border border-gray-100 px-4 py-3 flex-row items-center">
              ...
              <View className="flex-1">
                <Text className="text-sm font-bold text-black">{toastTitle}</Text>
                <Text className="text-xs text-gray-500 mt-0.5">{toastMsg}</Text>
              </View>

              <TouchableOpacity onPress={() => setShowToast(false)} className="p-1">
