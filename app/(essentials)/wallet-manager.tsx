...
export default function WalletManagerScreen() {
  ...
  const [wallets, setWallets] = useState<Wallet[]>([
    {
      ...
      valueUSD: "$25,847.32",
      icon: "₿",
      color: APP_COLORS.orangeBitcoin,
      isDefault: true,
    },
    {
      id: "2",
  ...
  const WalletRow = ({ w }: { w: Wallet }) => (
    <View className="bg-white dark:bg-slate-900 rounded-2xl border border-om-border dark:border-slate-700 mb-3 overflow-hidden">
      <View className="px-4 pt-4 pb-3">
          ...
          <View className="flex-row items-center flex-1 pr-3">
            ...
            <View className="flex-1">
              <View className="flex-row items-center">
                <Text
                  className="text-[15px] font-bold text-black dark:text-slate-100 flex-1"
                  numberOfLines={1}
                >
                  {w.name}
                </Text>
                ...
                {w.isDefault ? (
                  <View className="ml-2 px-2 py-1 rounded-md bg-om-accent/15 border border-om-accent/25">
          ...
          <View className="flex-row items-center">
            {!w.isDefault ? (
              <TouchableOpacity
                ...
                activeOpacity={0.9}
                className="w-9 h-9 rounded-full bg-om-bg dark:bg-slate-950 border border-om-border dark:border-slate-700 items-center justify-center mr-2"
              >
                <Ionicons
                  name="star-outline"
                  size={18}
        ...
        <View className="flex-row items-end justify-between mt-4">
          <View>
            <Text className="text-[20px] font-extrabold text-black dark:text-slate-100">
              {w.balance}{" "}
              <Text className="text-[16px] font-bold">{w.coin}</Text>
            </Text>
            <Text className="text-[12px] text-om-subtext dark:text-slate-400 mt-0.5">
              {w.valueUSD}
            </Text>
          ...
          >
            <Ionicons name="copy-outline" size={16} color={themeColors.text} />
            <Text className="ml-2 text-[12px] font-semibold text-black dark:text-slate-100">
              Copy
            </Text>
          </TouchableOpacity>
        </View>
      ...
      <View className="flex-row border-t border-om-border dark:border-slate-700">
        {[
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.9}
            className={[
              "flex-1 py-3 items-center justify-center bg-white dark:bg-slate-900",
              idx !== 3
                ? "border-r border-om-border dark:border-slate-700"
                : "",
            ].join(" ")}
          >
            <Ionicons name={a.icon} size={18} color={APP_COLORS.accent} />
            <Text className="text-[11px] mt-1 font-semibold text-om-text dark:text-slate-100">
              {a.label}
            </Text>
          </TouchableOpacity>
        ))}
  ...
  return (
    ...
    >
      ...
      >
        ...
        <View className="bg-white dark:bg-slate-900 rounded-2xl border border-om-border dark:border-slate-700 p-5 mb-4">
          <View className="flex-row items-center justify-between">
            <View>
              <Text className="text-[12px] text-om-subtext dark:text-slate-400 font-semibold">
                Total balance
              </Text>
              <Text className="text-[32px] font-extrabold text-black dark:text-slate-100 mt-1">
                {totalUSD}
              </Text>
            ...
            <View className="w-12 h-12 rounded-2xl bg-om-accent items-center justify-center">
              <Ionicons
                name="wallet-outline"
                size={22}
          ...
          <View className="flex-row gap-3 mt-5">
            {[
              ...
              <TouchableOpacity
                key={a.label}
                activeOpacity={0.9}
                className="flex-1 flex-row items-center justify-center bg-om-accent py-3 rounded-xl"
              >
                <Ionicons name={a.icon} size={18} color={themeColors.text} />
                <Text className="ml-2 font-bold text-black dark:text-slate-100">
                  {a.label}
                </Text>
              </TouchableOpacity>
            ))}
        ...
        <View className="mb-3">
          ...
          {wallets.length ? (
            ...
            <View className="items-center bg-white dark:bg-slate-900 border border-om-border dark:border-slate-700 rounded-2xl p-10">
              ...
              />
              <Text className="mt-4 text-[16px] font-bold text-black dark:text-slate-100">
                No wallets added
              </Text>
              <Text className="text-[13px] text-om-subtext dark:text-slate-400 mt-1 text-center">
      ...
      >
        <View className="flex-1 bg-black/50 justify-center items-center p-5">
          <View className="bg-white dark:bg-slate-900 w-full max-w-[420px] rounded-2xl border border-om-border dark:border-slate-700 overflow-hidden">
            <View className="px-5 py-4 border-b border-om-border dark:border-slate-700 flex-row items-center">
              <Text className="text-[16px] font-bold text-black dark:text-slate-100 flex-1">
                Add wallet
              </Text>
              <TouchableOpacity
            ...
            <View className="p-5">
              <Text className="text-[12px] font-semibold text-om-subtext dark:text-slate-400 mb-2">
                Wallet name
              </Text>
              <TextInput
                ...
                className="bg-om-bg dark:bg-slate-950 border border-om-border dark:border-slate-700 rounded-xl px-4 py-3 text-[14px] text-black dark:text-slate-100"
              />

              <Text className="text-[12px] font-semibold text-om-subtext dark:text-slate-400 mt-4 mb-2">
                Coin
              </Text>
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      className={[
                        ...
                        active
                          ? "bg-om-accent/10 border-om-accent/30"
              ...
              <TextInput
                ...
                placeholderTextColor={APP_COLORS.placeholder}
                multiline
                className="bg-om-bg dark:bg-slate-950 border border-om-border dark:border-slate-700 rounded-xl px-4 py-3 h-24 text-[14px] text-black dark:text-slate-100"
              />

              <TouchableOpacity
                onPress={handleAddWallet}
                activeOpacity={0.9}
                className="bg-om-accent rounded-xl py-4 items-center mt-5"
              >
                <Text className="text-[14px] font-extrabold text-black dark:text-slate-100">
                  Save wallet
                </Text>
              </TouchableOpacity>
              ...
              <TouchableOpacity
                onPress={() => setShowAdd(false)}
                activeOpacity={0.9}
                className="rounded-xl py-4 items-center mt-3 border border-om-border dark:border-slate-700 bg-white dark:bg-slate-900"
              >
                <Text className="text-[14px] font-bold text-om-text dark:text-slate-100">
                  Cancel
                </Text>
              </TouchableOpacity>
            </View>
